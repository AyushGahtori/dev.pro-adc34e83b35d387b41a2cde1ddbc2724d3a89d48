export default function Projects({ name, img, alt, type, link, year, tools, GitHub }) {
  return (
    <div>
      <a
        target="_blank"
      ...
      <div className="mt-4">
        <div className="flex space-x-2 mb-3">
          ...
          {/* Check if GitHub is an array or a string */}
          {Array.isArray(GitHub) ? (
            GitHub.map((githubLink, index) => (
              <a
                key={index}
                href={githubLink}
                target="_blank"
                rel="noopener noreferrer"
                className="rounded-full bg-transparent border border-secondary-600 flex justify-center items-center px-4 py-1 text-blue-500 text-body-4 2xl:text-3xl"
              >
                GitHub-{index + 1}
              </a>
            ))
